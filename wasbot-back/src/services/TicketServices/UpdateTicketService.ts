...
const UpdateTicketService = async ({
  ...
  try {
    ...
    const promptNew: string | null = ticketData.promptNew || null;
    ...
    if (promptNew) {
      ...
      if (!threadId) {
        const { data } = await axios.post(`https://api.openai.com/v1/threads`, {}, {
          headers: {
            'authorization': 'Bearer sk-yAoW6JLUJUt3USYep7CL61U3UIis3JtzNPaEnr8WnsT3BlbkFJU8njR3TiwgS8455SmJvQkVvdXVMBApSHhhcEMmDFMA',
            'OpenAI-Beta': 'assistants=v2'
          }
        });
        threadId = data.id;
      }
      await ticket.update({ threadId, promptNew })
    }
