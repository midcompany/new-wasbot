...
export async function sendPromptMessage(
  ...
  console.log("[SEND-PROMPT] - ", ticket.id, ticket.companyId, threadId, prompt.apiKey, prompt.prompt, realTicket.status, new Date());
    ...
    try {
      ...
      if (assistantMessage) {
        ...
        if (!callbackTracker.has(callbackKey)) {
          ...
          console.log("Callback já chamado para este ticket e body. Ignorando...");
        }

        return assistantMessage;
      }
      console.log("NAO TENHO MENSAGENS - ", assistantMessage);
    } catch (error) {
