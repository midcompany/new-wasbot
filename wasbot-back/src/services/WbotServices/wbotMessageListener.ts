...
} from "../../middleware/cacheMid";
import axios from "axios";
import { sendPromptMessage } from "./sendPrompMessage";
...
async function textToSpeechToFileUsingElevenLabs(
  text: string,
  voiceId: string,
...
const handleOpenAi = async (msg, wbot, ticket, contact, mediaSent) => {
  ...
  console.log("[HANDLE OPEN AI] - [MSG] - ", msg);

  if ((!bodyMessage && !msg.message?.imageMessage) || msg?.key.fromMe) return;
  ...
  if (!prompt || msg.messageStubType) return;
  ...
  // await wbot.sendPresenceUpdate('composing', `${ticket?.contact?.number}@s.whatsapp.net`);

  const sendMessageWithDelay = async responseText => {
    ...
    if (assistantMode === "texto" || assistantMode === "text") {
      if (imageMatch) {
        ...
        if (imageUrl) {
          ...
          try {
            const filePath = await downloadImageToFile(cleanedImageUrl, randomFileName, `${publicFolder}/images`);
      ...
      if (imageMatch) {
        ...
        responseText = responseText
          ...
          .replace(/^audio:/i, "")
          .trim();
        await handleAudioResponse(responseText, parsedKeys);
  ...
  const handleAudioResponse = async (responseText, parsedKeys: any) => {
  ...
  if (msg.message?.imageMessage) {
    const caption = msg.message.imageMessage.caption || "Segue a imagem";
    ...
    const imageUrl = `https://api.tzsexpertacademy.com/public/${media.filename}`;
    ...
    // await sendMessageWithDelay(response);
  } else if (
    msg.message?.conversation ||
    msg.message?.extendedTextMessage?.text
  ) {
    let response = await sendPromptMessage(
      bodyMessage,
      ticket,
      wbot,
      sendMessageWithDelay
...
export const verifyMessage = async (
  ...
  await updateCacheWithNewMessage(
    `/messages/${ticket.id}?pageNumber=1`,
    message
  );
  if (!msg.key.fromMe && ticket.status === "closed") {
    await ticket.update({ status: "pending" });
...
const verifyQueue = async (
  ...
  if (!msg.key.fromMe && scheduleType) {
    if (
      ...
      console.log("NAO ENVIANDO MENSAGEM, FORA DO EXPEDIENTE VIA COMPANY");
    ...
    if (scheduleType.value === "queue" && ticket.queueId !== null) {
      ...
      if (
        ...
        if (now.isBefore(startTime) || now.isAfter(endTime)) {
          console.log("NAO ENVIANDO MENSAGEM, FORA DO EXPEDIENTE VIA FILA");
          return;
        }
      }
    }
  ...
  if (queues.length === 1) {
    ...
    if (!msg.key.fromMe && !ticket.isGroup && !isNil(queues[0]?.promptId)) {
      ...
      const promptFromQueue = await ShowPromptService({
        companyId,
        promptId: queues[0]?.promptId
      });
      const hasMedia =
      ...
      if (hasMedia) {
        ...
        console.log("[VERIFY QUEUE] - ", mediaSent);
        ...
        await handleOpenAi(
          ...
          wbot,
          {
            ...
            queue: { ...queues[0], prompt: promptFromQueue }
    ...
    await UpdateTicketService({
      ticketData: { queueId: firstQueue.id, chatbot, status: "open" },
      ticketId: ticket.id,
      companyId: ticket.companyId
    });
...
const handleMessage = async (
  ...
  try {
    ...
    if (hasMedia) {
      mediaSent = await verifyMediaMessage(msg, ticket, contact);
      console.log("[handleMessage] - MEDIASENT", mediaSent, msg);
      await handleOpenAi(msg, wbot, ticket, contact, mediaSent);
    } else {
      await verifyMessage(msg, ticket, contact);
      if (!msg.key.fromMe && ticket?.queue?.prompt) {
        try {
          // const body = getBodyMessage(msg);
          // await sendPromptMessage(body, ticket, wbot)
          await handleOpenAi(msg, wbot, ticket, contact, mediaSent);
        } catch (err) {
          console.log(err);
        }
...
const handleMsgAck = async (
  ...
  try {
    ...
    await updateCacheWithNewMessage(
      `/messages/${message.ticketId}?pageNumber=1`,
      messageToUpdated
    );
