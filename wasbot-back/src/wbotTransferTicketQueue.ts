...
export const TransferTicketQueue = async (): Promise<void> => {
  ...
  tickets.forEach(async ticket => {
    ...
    if (!wpp || !wpp.transferQueueId) return;
    ...
    if (new Date() > dataLimite) {
      ...
      const ticketWithQueue = await ShowTicketService(ticket.id, ticket.companyId);
      ...
      const data = {
          key: {
            ...
            fromMe: false,
            id: 'D0165836A6BF6177D1E1943415A71EA8'
          },
          messageTimestamp: 1730473322,
          broadcast: false,
          message: {
            conversation: ticket.lastMessage,
          },
        }
      console.log('+++',wbot.id,  ticketWithQueue.companyId)
      
      console.log("")
      console.log(contact)
     try {

      await verifyQueue(wbot, data, ticketWithQueue, contact);
     } catch(err:any) {
